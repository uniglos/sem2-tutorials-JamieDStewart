...
#pragma region Main Loop
	...
	while (msg.message != WM_QUIT)
	{
		...
		FillRect(gBufferDevContex, &clRect, (HBRUSH)(COLOR_WINDOW + 2));//FillRect will fill a rect with a value, clear the window

		for (int i = 0; i < windowHeight; ++i)					//iterate over each row of the bitmap buffer
		{
			for (int j = 0; j < windowWidth; ++j)				//iterate over each pixel in the row
			{
				//calculate colour value as a float in range 0 -> 1 to produce a gradient
				float r = float(j) / (windowWidth - 1);
				float g = float(i) / (windowWidth - 1);
				float b = 1.f - r * g;
