...
#pragma endregion
	...
	mainCamera.LookAt(Vector3(0.f, 0.f, -1.f), Vector3(0.f, 1.f, 0.f));
	...
	Ellipsoid s1(Vector3(0.f, 0.f, -1.5f), 0.5f);
	...
	for (int i = 0; i < imageHeight; ++i)
	{
		...
		for (int j = 0; j < imageWidth; ++j )
		{
			...
			{
				//calculate lighting
				rayColour = dl.CaclulateLighting(hitPos, mainCamera.GetPosition(), surfNormal);
			}
			else
			{
